...
test('planDataverseSynthesis maps the approval example into entities, existing forms, and JS behaviors', () => {
  ...
  assert.equal(plan.portalRuntime?.bootstrap.identityMode, 'generic-profile');
  assert.equal(plan.portalRuntime?.bootstrap.routes.entryPath, '/approval-request');
  assert.equal(plan.portalRuntime?.bootstrap.routes.statusPath, '/approval-request/status');
  assert.deepEqual(
  ...
  assert.equal(plan.portalRuntime?.hostPackageName, 'dbm-portal-runtime');
});
...
test('planDataverseSynthesis exposes R2.1 authoring table contracts', () => {
  const plan = planDataverseSynthesis(approvalRequestModel as DbmModelV1);

  assert.deepEqual(
    plan.authoringTables.map((table) => table.logicalName),
    [
      'dbm_authoringunit',
      'dbm_authoringdraft',
  ...
  assert.deepEqual(
    columnsByTable.get('dbm_authoringunit'),
    [
      'dbm_authoringunitid',
      'dbm_name',
      'dbm_unittype',
  ...
  assert.deepEqual(
    columnsByTable.get('dbm_authoringdraft'),
    [
  ...
  assert.deepEqual(
    columnsByTable.get('dbm_editlock'),
    [
      'dbm_editlockid',
      'dbm_name',
      'dbm_targettype',
  ...
  assert.deepEqual(
    columnsByTable.get('dbm_designersession'),
    [
...
test('planDataverseSynthesis exposes R2.1 authoring operation contracts', () => {
  ...
  assert.deepEqual(
    plan.authoringOperations.map((operation) => operation.name),
    [
      'acquire-lock',
      'renew-lock',
      'release-lock',
  ...
  assert.deepEqual(
    plan.authoringOperations.find((operation) => operation.name === 'autosave-draft'),
    {
      name: 'autosave-draft',
