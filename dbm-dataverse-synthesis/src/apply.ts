import {
  buildLabel,
  buildRequiredLevel,
  DEFAULT_GENERATED_METADATA_SOLUTION_PUBLISHER_UNIQUE_NAME,
...
function decodeUtf8Base64(value: string | null | undefined): string {
  if (!value) {
    return '';
  }
...
function getDataverseErrorMessage(result: DataverseResponse, fallback: string): string {
  const nestedMessage =
...
function buildRelationshipBody(
  ...
  referencedEntity: DataverseEntityPlan,
  referencingEntity: DataverseEntityPlan,
  lookupColumn: DataverseColumnPlan,
  languageCode: number
  ...
  return {
    ...
    AssociatedMenuConfiguration: {
      ...
      Group: 'Details',
      Label: buildLabel(`${referencedEntity.displayName}s`, languageCode),
      Order: 10000
    },
...
async function tryGetExistingPublisher(
  ...
  const result = await dataverseRequest(
    ...
    accessToken,
    'GET',
    `publishers?$select=publisherid,uniquename&$filter=${filter}`
  );
...
async function ensureGeneratedMetadataSolution(
  ...
  const existing = await dataverseRequest(
    ...
    accessToken,
    'GET',
    `solutions?$select=solutionid,uniquename,version&$filter=${filter}`
  );
  ...
  if (!existing.ok) {
    throw new Error(
      getDataverseErrorMessage(
        existing,
        `Failed to query Dataverse solution '${plan.generatedMetadataSolutionName}'.`
      )
    );
  }
  ...
  const publisher = await tryGetExistingPublisher(
    environmentConfig,
    accessToken,
    DEFAULT_GENERATED_METADATA_SOLUTION_PUBLISHER_UNIQUE_NAME
  );
  if (!publisher) {
    throw new Error(
      `Dataverse publisher '${DEFAULT_GENERATED_METADATA_SOLUTION_PUBLISHER_UNIQUE_NAME}' was not found. Bootstrap the core solution first.`
    );
  }

  const createResult = await dataverseRequest(
    environmentConfig.dataverseUrl,
    accessToken,
    'POST',
    'solutions',
    {
      ...
      version: DEFAULT_GENERATED_METADATA_SOLUTION_VERSION,
  ...
  if (!createResult.ok) {
    throw new Error(
      getDataverseErrorMessage(
        createResult,
        `Failed to create Dataverse solution '${plan.generatedMetadataSolutionName}'.`
      )
    );
  }
...
async function ensureColumn(
  ...
  if (existing.status !== 404) {
    throw new Error(
      getDataverseErrorMessage(
        existing,
        `Failed to query column '${column.logicalName}' on entity '${entity.logicalName}'.`
      )
    );
  }
  ...
  if (!result.ok) {
    throw new Error(
      getDataverseErrorMessage(
        result,
        `Failed to create column '${column.logicalName}' on entity '${entity.logicalName}'.`
      )
    );
  }
...
async function ensureRelationship(
  ...
  const lookupColumn = referencingEntity?.columns.find(
    (column) => column.logicalName === relationship.referencingAttributeLogicalName
  );

  if (!referencedEntity || !referencingEntity || !lookupColumn) {
    throw new Error(
      `Relationship '${relationship.logicalName}' could not resolve the referenced entity, referencing entity, or lookup column from the synthesis plan.`
    );
  }
  ...
  const createResult = await dataverseRequest(
    ...
    buildRelationshipBody(relationship, referencedEntity, referencingEntity, lookupColumn, languageCode),
    solutionName
  );
...
async function ensureBehaviorWebResource(
  behavior: DataverseBehaviorPlan,
  environmentConfig: DataverseEnvironmentConfig,
  accessToken: string,
  solutionName: string,
  actions: DataverseApplyAction[]
  ...
  const existing = await dataverseRequest(
    ...
    accessToken,
    'GET',
    `webresourceset?$select=webresourceid,name,content,displayname,webresourcetype&$filter=${filter}`
  );
  ...
  const desiredContent = normalizeTextContent(behavior.content);
  const existingContent = normalizeTextContent(decodeUtf8Base64(existingResource?.content));
  ...
  if (
    existingResource?.webresourceid &&
  ...
  const result = existingResource?.webresourceid
    ? await dataverseRequest(
      ...
      accessToken,
      'PATCH',
      `webresourceset(${existingResource.webresourceid})`,
      body,
      solutionName
    )
    : await dataverseRequest(
...
async function ensureForm(
  ...
  const existing = await dataverseRequest(
    ...
    accessToken,
    'GET',
    `systemforms(${normalizedFormId})?$select=formid,name,formxml`
  );
  ...
  if (normalizeTextContent(currentFormXml) === normalizeTextContent(patchedFormXml)) {
    actions.push({
      componentType: 'form',
      logicalName: form.systemFormId,
      state: 'skipped',
...
export async function applySynthesisPlanToDev(
  ...
  const relationshipBackedLookupColumns = new Set(
    plan.relationships
  ...
  for (const relationship of plan.relationships) {
    await ensureRelationship(
      ...
      environmentConfig,
      auth.accessToken,
      plan.generatedMetadataSolutionName,
      actions,
      languageCode
    );
  }
  ...
  for (const behavior of plan.behaviors) {
    await ensureBehaviorWebResource(
      behavior,
      environmentConfig,
      auth.accessToken,
      plan.generatedMetadataSolutionName,
