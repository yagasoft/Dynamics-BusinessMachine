...
function createSyntheticRuntimeStateColumns(entityId: string, entityLogicalName: string): DataverseColumnPlan[] {
  const prefix = getPublisherPrefix(entityLogicalName);
  const fieldDefinitions: Array<{ id: string; logicalName: string; displayName: string; readOnly: boolean }> = [
...
function isSupportedPortalEntryField(
  field: DbmFieldV1
): field is DbmFieldV1 & { dataType: DbmPortalRuntimeEntryFieldV1['dataType'] } {
  return (
...
function buildPortalRuntimeEntryFields(
  model: DbmModelV1,
  runtimeOwnerEntityId: string,
  startStageId: string,
  startFormId: string,
  diagnostics: DataverseSynthesisDiagnostic[]
): DbmPortalRuntimeEntryFieldV1[] {
  const runtimeOwnerEntity = getEntityById(model, runtimeOwnerEntityId);
  const startForm = model.forms.find((form) => form.id === startFormId);
  if (!runtimeOwnerEntity || !startForm) {
    diagnostics.push(
      createDiagnostic(
        'portal-runtime-entry-fields-incomplete',
        'warning',
        'Portal runtime entry fields could not be derived from the start-form configuration.',
        'forms'
      )
    );
    return [];
  ...
  if (startFormStateIds.size === 0) {
    diagnostics.push(
      createDiagnostic(
        'portal-runtime-entry-fields-missing-form-states',
        'warning',
        'Portal runtime entry fields could not be derived because the start-stage steps do not declare form states.',
        'process.steps'
      )
    );
    return [];
  ...
  const hintsByElementId = new Map<string, string>();
  ...
  const entryFields: DbmPortalRuntimeEntryFieldV1[] = [];
  const emittedLogicalNames = new Set<string>();
  for (const element of startForm.elements) {
    ...
    const field = getFieldById(runtimeOwnerEntity, element.binding.fieldId);
    ...
    const logicalName = tryGetLogicalName(
      ...
      `field '${field.id}'`,
      diagnostics,
      `metadata.entities.${runtimeOwnerEntity.id}.fields.${field.id}`
    );
...
function buildPortalRuntimePlan(
  model: DbmModelV1,
  entityPlans: Map<string, DataverseEntityPlan>,
  runtimeOwnerEntityId: string | null,
  diagnostics: DataverseSynthesisDiagnostic[]
): DataversePortalRuntimePlan | null {
  if (!runtimeOwnerEntityId) {
    diagnostics.push(
      createDiagnostic(
        'portal-runtime-missing-owner-entity',
        'warning',
        'Portal runtime bootstrap was skipped because the process owner entity could not be resolved.',
        'process.stages'
      )
    );
    return null;
  ...
  if (!runtimeOwnerEntity || !startStage || !defaultStep || !startStage.formId) {
    diagnostics.push(
      createDiagnostic(
        'portal-runtime-bootstrap-incomplete',
        'warning',
        'Portal runtime bootstrap was skipped because the start-stage runtime contract is incomplete.',
        'process.stages'
      )
    );
    return null;
  ...
  const prefix = getPublisherPrefix(runtimeOwnerEntity.logicalName);
  const routeSegment = toPortalRouteSegment(model.package.id);
  const entryFields = buildPortalRuntimeEntryFields(
    model,
    runtimeOwnerEntityId,
    startStage.id,
    startStage.formId,
    diagnostics
  );
...
function createSolutionMetadataColumns(): DataverseAuthoringColumnPlan[] {
  return [
