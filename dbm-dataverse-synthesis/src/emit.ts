...
function createSolutionXml(plan: DataverseSynthesisPlan): string {
  ...
  manifest.ele('Version').txt(DEFAULT_GENERATED_METADATA_SOLUTION_VERSION);
...
function createRelationshipsIndex(relationships: DataverseRelationshipPlan[]): string {
  const root = create({ version: '1.0', encoding: 'utf-8' }).ele('EntityRelationships', {
    'xmlns:xsi': 'http://www.w3.org/2001/XMLSchema-instance'
  });
...
function addCommonAttributeFlags(attribute: any, isPrimaryName: boolean, isPrimaryKey: boolean): void {
...
function addDisplayNodes(attribute: any, displayName: string, description: string): void {
...
function writePrimaryKeyAttribute(attributes: any, entity: DataverseEntityPlan): void {
...
function getXmlAttributeType(column: DataverseColumnPlan): string {
...
function writeColumnAttribute(attributes: any, entity: DataverseEntityPlan, column: DataverseColumnPlan): void {
  ...
  attribute.ele('DisplayMask').txt(
    isPrimaryName
      ? 'PrimaryName|ValidForAdvancedFind|ValidForForm|ValidForGrid|RequiredForForm'
      : 'ValidForAdvancedFind|ValidForForm|ValidForGrid'
  );
  attribute.ele('ImeMode').txt(
    column.attributeType === 'Money' || column.attributeType === 'Integer' || column.attributeType === 'Decimal'
      ? 'disabled'
      : 'auto'
  );
  ...
  switch (column.attributeType) {
    ...
    case 'Picklist':
      appendOptionSet(
        attribute,
        'picklist',
        `${entity.logicalName}_${column.logicalName}`,
        column.displayName,
