...
exports.HTML_WEB_RESOURCE_CLASS_ID = exports.SHARED_PROCESS_EXPERIENCE_HOST_PAGE_WEB_RESOURCE_NAME = exports.SHARED_PROCESS_EXPERIENCE_HOST_PAGE_BEHAVIOR_ID = exports.SHARED_PROCESS_EXPERIENCE_RENDERER_WEB_RESOURCE_NAME = exports.SHARED_PROCESS_EXPERIENCE_RENDERER_BEHAVIOR_ID = exports.SHARED_FORM_RUNTIME_WEB_RESOURCE_NAME = exports.SHARED_FORM_RUNTIME_BEHAVIOR_ID = void 0;
...
exports.SHARED_PROCESS_EXPERIENCE_RENDERER_BEHAVIOR_ID = 'dbm-process-experience-renderer';
exports.SHARED_PROCESS_EXPERIENCE_RENDERER_WEB_RESOURCE_NAME = 'ys_/dbm/process-experience/renderer.js';
exports.SHARED_PROCESS_EXPERIENCE_HOST_PAGE_BEHAVIOR_ID = 'dbm-process-experience-host-page';
exports.SHARED_PROCESS_EXPERIENCE_HOST_PAGE_WEB_RESOURCE_NAME = 'ys_/dbm/process-experience/host.html';
exports.HTML_WEB_RESOURCE_CLASS_ID = '{9FDF5F91-88B1-47f4-AD53-C11EFC01A01D}';
...
function getDefaultFormStateId(model, formId) {
    const stageMap = new Map((0, process_portfolio_1.getProcessStages)(model).map((stage) => [stage.id, stage]));
    for (const step of (0, process_portfolio_1.getProcessSteps)(model)) {
...
function getRuntimeStateFieldLogicalNames(entityPlan) {
...
function buildRuntimeValueBindings(model) {
    const bindings = [];
...
function buildProcessExperienceRuntimeModel(model) {
    return (0, process_portfolio_1.buildProcessExperienceRuntimeModelFromModel)(model);
}
function buildStateJumpTarget(formPlan, state) {
    const targetControlName = state.visibleControlNames[0] ?? null;
    if (!targetControlName) {
        return null;
    }
    const section = formPlan.sections.find((candidate) => candidate.controls.some((control) => control.controlName === targetControlName));
    if (!section) {
        return null;
    }
    return {
...
function buildProcessHostConfig(formPlan, displayName, model, rendererVersionKey, hostPageVersionKey) {
    ...
    const jumpTargetsByFormStateId = Object.fromEntries(formPlan.states
        .map((state) => {
    ...
    return {
        ...
        processId: (0, process_portfolio_1.getMainProcess)(model).id,
        ...
        supported: {
            ...
            sectionId: (0, common_1.createDeterministicGuid)(`${formPlan.id}:process-host-section`),
            cellId: (0, common_1.createDeterministicGuid)(`${formPlan.id}:process-host-cell`),
            controlName: `WebResource_dbmProcessHost_${safeFormId}`,
            webResourceName: exports.SHARED_PROCESS_EXPERIENCE_HOST_PAGE_WEB_RESOURCE_NAME,
            webResourceId: (0, common_1.createDeterministicGuid)(exports.SHARED_PROCESS_EXPERIENCE_HOST_PAGE_WEB_RESOURCE_NAME),
...
function readSharedProcessExperienceRendererContent() {
    let packageRoot = __dirname;
    while (node_path_1.default.basename(packageRoot) !== 'dbm-dataverse-synthesis') {
        const parent = node_path_1.default.dirname(packageRoot);
        if (parent === packageRoot) {
            throw new Error('Unable to locate the dbm-dataverse-synthesis package root.');
        }
        packageRoot = parent;
    }
    const repoRoot = node_path_1.default.dirname(packageRoot);
    const bundlePath = node_path_1.default.join(repoRoot, 'dbm-process-experience', 'dist', 'browser', 'renderer.js');
    return (0, node_fs_1.readFileSync)(bundlePath, 'utf8');
}
function buildProcessHostPageContent() {
    return [
        '<!doctype html>',
        '<html lang="en">',
        '<head>',
...
function getFormPrimaryBinding(form) {
    return form.entityBindings.find((binding) => binding.id === form.primaryEntityBindingId);
}
function getPrimaryIdLogicalName(entity, entityPlan) {
...
function buildStageHandoffsByStageId(model, entityPlans, diagnostics) {
    const transitions = (0, process_portfolio_1.getProcessTransitions)(model);
    const stepTransitions = (0, process_portfolio_1.getProcessStepTransitions)(model);
    const stageMap = new Map((0, process_portfolio_1.getProcessStages)(model).map((stage) => [stage.id, stage]));
    const stepMap = new Map((0, process_portfolio_1.getProcessSteps)(model).map((step) => [step.id, step]));
    ...
    const handoffs = {};
    const registerHandoff = (sourceStageId, targetStageId, subjectHandoff) => {
        if (!subjectHandoff) {
            return;
        }
        ...
        if (!sourceStage || !targetStage || !sourceEntityPlan || !targetEntityPlan || !sourceEntity || !targetEntity) {
            diagnostics.push((0, common_1.createDiagnostic)('missing-stage-handoff-binding', 'error', `Target stage '${targetStageId}' could not resolve source/target entity bindings for runtime handoff.`, `process.stages.${targetStageId}`));
            return;
        }
        ...
        const referencingEntity = relationship
            ? relationship.relationshipType === 'one-to-many'
                ...
                : entityMap.get(relationship.fromEntityId)
            : null;
        const referencingField = relationship?.referencingFieldId && referencingEntity
            ? (0, common_1.getFieldById)(referencingEntity, relationship.referencingFieldId)
            : undefined;
        const referencingFieldDataverseBinding = referencingField?.providerBindings.dataverse;
        ...
        if (current && JSON.stringify(current) !== JSON.stringify(nextPlan)) {
            diagnostics.push((0, common_1.createDiagnostic)('conflicting-target-stage-handoff', 'warning', `Target stage '${targetStageId}' is reached through conflicting cross-entity handoff definitions. The first definition will be used in the generated runtime.`, `process.stages.${targetStageId}`));
            return;
        }
    ...
    stepTransitions.forEach((transition) => {
        ...
        const targetStageId = 'stageId' in transition.target
...
function buildFormRuntimePlan(model, form, entityPlans, primaryEntityPlan, diagnostics) {
    const runtimeOwnerEntityId = getRuntimeOwnerEntityId(model);
    if (!runtimeOwnerEntityId) {
        diagnostics.push((0, common_1.createDiagnostic)('missing-runtime-owner-entity', 'error', 'Could not determine the runtime owner entity from the start-stage form.', `forms.${form.id}`));
        return null;
    }
    ...
    if (!runtimeOwnerLogicalName || !runtimeOwnerEntityPlan || !primaryEntityPlan) {
        diagnostics.push((0, common_1.createDiagnostic)('missing-runtime-form-binding', 'error', `Form '${form.id}' could not resolve runtime owner or primary entity bindings.`, `forms.${form.id}`));
        return null;
    }
    ...
    const process = (0, process_portfolio_1.getMainProcess)(model);
    const stages = (0, process_portfolio_1.getProcessStages)(model);
    ...
    const relatedProcessOwnerLookupFieldLogicalName = primaryEntityPlan.id === runtimeOwnerEntityId
        ? null
        : (() => {
            ...
            const field = primaryEntity ? (0, common_1.getFieldById)(primaryEntity, relationship.referencingFieldId) : undefined;
            return field?.providerBindings.dataverse?.logicalName?.trim() ?? null;
        })();
    const valueBindings = buildRuntimeValueBindings(model);
    const processExperienceRuntime = (0, process_portfolio_1.hasProcessPortfolio)(model) ? model : buildProcessExperienceRuntimeModel(model);
    ...
    return {
        ...
        stages: stages.map((stage) => ({
            ...
            stageType: (0, process_portfolio_1.getStageType)(stage),
            formId: stage.formId,
            entityLogicalName: stage.formId
                ? (() => {
        ...
        rules: Object.fromEntries(model.rules
...
function createFormPlan(model, form, entityPlans, diagnostics, rendererVersionKey, hostPageVersionKey) {
    ...
    const libraries = [
        {
            name: exports.SHARED_PROCESS_EXPERIENCE_RENDERER_WEB_RESOURCE_NAME,
            libraryUniqueId: (0, common_1.createDeterministicGuid)(`${form.id}:process-renderer-library`)
        },
        {
    ...
    const behaviors = [
        {
            ...
            webResourceName: exports.SHARED_PROCESS_EXPERIENCE_RENDERER_WEB_RESOURCE_NAME,
            webResourceId: (0, common_1.createDeterministicGuid)(exports.SHARED_PROCESS_EXPERIENCE_RENDERER_WEB_RESOURCE_NAME),
            supported: true,
            reason: null,
            webResourceType: 3,
        ...
        {
            id: exports.SHARED_PROCESS_EXPERIENCE_HOST_PAGE_BEHAVIOR_ID,
