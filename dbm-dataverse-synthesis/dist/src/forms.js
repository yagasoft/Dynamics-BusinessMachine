    ...
    const registerHandoff = (sourceStageId, targetStageId, subjectHandoff) => {
        ...
        const referencingEntity = relationship
            ? relationship.relationshipType === 'one-to-many'
                ...
                : entityMap.get(relationship.fromEntityId)
            : null;
        const referencingAttributeLogicalName = relationship?.referencingFieldId && referencingEntity
            ? (0, common_1.getFieldById)(referencingEntity, relationship.referencingFieldId)?.providerBindings.dataverse?.logicalName?.trim() ?? null
            : null;
        const nextPlan = {
            sourceStageId,
            targetStageId,
