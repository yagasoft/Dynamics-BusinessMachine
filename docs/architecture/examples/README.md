...
The matrix proves that `DbmModelV1` accepts user-defined process types, actor roles, stage kinds, work kinds, statuses, outcomes, nested child process definitions, stage-owned child process links, visibility rules, and portal projection contracts without making a single domain example authoritative.

- `linear-service-fulfilment.model.json`: simple portal-to-back-office-to-portal flow.
- `employee-onboarding.model.json`: parent, child, and grandchild process definitions for onboarding, IT readiness, and access review.
...
- `field-inspection.model.json`: scheduling, visit, evidence capture, closure, and portal-safe projection.
